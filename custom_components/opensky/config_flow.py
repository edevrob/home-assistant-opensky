...
from .api import OpenSkyApiClient, OpenSkyApiUnauthenticatedError
from .const import (
    CONF_ALTITUDE,
...
class OpenSkyOptionsFlowHandler(OptionsFlow):
    ...
    async def async_step_init(
        ...
        if user_input is not None:
            authentication = (
                CONF_CLIENT_ID in user_input or CONF_CLIENT_SECRET in user_input
            )
            if authentication and CONF_CLIENT_ID not in user_input:
                errors["base"] = "client_id_missing"
            if authentication and CONF_CLIENT_SECRET not in user_input:
                errors["base"] = "client_secret_missing"
            if user_input[CONF_CONTRIBUTING_USER] and not authentication:
                errors["base"] = "no_authentication"
            if authentication and not errors:
                opensky = OpenSkyApiClient(session=async_get_clientsession(self.hass))
                try:
                ...
                except OpenSkyApiUnauthenticatedError:
